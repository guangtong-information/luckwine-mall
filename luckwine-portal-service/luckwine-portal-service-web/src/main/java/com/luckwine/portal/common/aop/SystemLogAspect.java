...
public class SystemLogAspect {
    ...
    @Pointcut("@annotation(com.luckwine.portal.common.annotation.SystemLog)")
        ...
        try {
            ...
            if (StrUtil.isNotBlank(username)) {
                ...
                if(esRecord){
                    ...
                    //日志标题
                    logObj.setName(getControllerMethodDescription(joinPoint));
                    //日志请求url
                    logObj.setRequestUrl(request.getRequestURI());
                    //请求方式
                    logObj.setRequestType(request.getMethod());
                    //请求参数
                    Map<String,String[]> logParams=request.getParameterMap();
                    logObj.setMapToParams(logParams);
                    //请求用户
                    logObj.setUsername(username);
                    //请求IP
                    logObj.setIp(IpInfoUtil.getIpAddr(request));
                    //IP地址
                    logObj.setIpInfo(IpInfoUtil.getIpCity(IpInfoUtil.getIpAddr(request)));
                    //请求开始时间
                    Date logStartTime = beginTimeThreadLocal.get();
                    ...
                    //请求耗时
                    Long logElapsedTime = endTime - beginTime;
                    logObj.setCostTime(logElapsedTime.intValue());
