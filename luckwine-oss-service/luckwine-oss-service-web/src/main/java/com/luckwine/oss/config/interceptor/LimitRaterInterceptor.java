package com.luckwine.oss.config.interceptor;
...
import com.luckwine.oss.exception.OSSException;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${oss.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${oss.rateLimit.limit}")
    private Integer limit;

    @Value("${oss.rateLimit.timeout}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (rateLimiter != null) {
            ...
            if (StrUtil.isBlank(token3)) {
                throw new OSSException("当前访问人数太多啦，请稍后再试");
            }
        }
        return true;
    }
